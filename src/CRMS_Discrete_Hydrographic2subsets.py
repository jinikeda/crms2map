...
CRMS_pore = CRMS_pore.iloc[:,[0,5,14,36,37]] # Select CPRA Station ID, Measurement Depth (ft), Soil Porewater Salinity (ppt),Latitude, and Longitude
...
CRMS_pore['CPRA Station ID'] = CRMS_pore['CPRA Station ID'].str.replace('-P\d+', '', regex=True)
...
# Create pivot table
pivoted_10 = depth_10_pore.pivot_table(index=depth_10_pore.index, columns='CPRA Station ID', values='Soil Porewater Salinity (ppt)')
pivoted_30 = depth_30_pore.pivot_table(index=depth_30_pore.index, columns='CPRA Station ID', values='Soil Porewater Salinity (ppt)')
...
pivoted_10.describe().to_csv(os.path.join(Processspace,'Pore_salinity_10_summary.csv'))
pivoted_30.describe().to_csv(os.path.join(Processspace,'Pore_salinity_30_summary.csv'))
